...
[caption="Figure 1: "]
.Figure 1. Kubernetes Client A invoking Kubernetes Service C.
...
//if you prefer a canonical name instead of the IP address, then do a DNS lookup
//which return a string formatted like
...
Note that the IP address returned, and the canonical address of `<service-name>.<k8s-domain>` can only be used on the kubernetes cluster and can not be approached externally. 
...
[caption="Figure 2: "]
.Figure 2. External Client A invoking Kubernetes Service C.

== Scenario 3 - K8s Client to K8s Service via the APIMan Gateway

image::k8s-service-scenario-3.png[]
[caption="Figure 3: "]
.Figure 3. Kubernetes Client A invoking Kubernetes Service C via the APIMan Gateway.

== Scanario 4 - External Client to K8s Service via the APIMan Gateway

image::k8s-service-scenario-4.png[]
[caption="Figure 4: "]
.Figure 4. External Client A invoking Kubernetes Service C via the APIMan Gateway.